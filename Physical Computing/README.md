...
| --- | --- |
| Objects<br>`MMButton`<br>`MMKnob`<br>`MMStick` | Button: push buttons, switches etc.<br>Knob: knobs, dials, rotary encoders etc.<br>Stick: joystick, thumbsticks etc. |
| Creating an object | First, declare object, e.g. <br>`MMButton button`<br><br>Then, in setup, define object, e.g.<br>`button = new MMButton("BUTTON 1", 0, false, 0.5, 50, 50, 80, 200);`  |
| Updating and displaying object | in draw loop<br><br>update object with serial data, e.g.<br>`button.update(SERIAL_DATA);`<br><br>then display object<br>`button.display();` |
...
| `MMStick` fields | `val` *(PVector)*: an array of 2D vectors with the last x number of sensor readings.<br>`val[0]` *(PVector)*: Most recent sensor reading.<br>`val[i].x` *(float)*: first sensor reading.<br>`val[i].y` *(float)*: second sensor reading.<br>`start` *(PVector)*: minimum sensor readings.<br>`end` *(PVector)*: maximum sensor readings<br>`center_point` *(PVector)*: stick's center point.<br> |
